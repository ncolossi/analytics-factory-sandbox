...
```
evals/
├── golden/definitions/{bronze,silver,gold}/...   # must pass clean
└── cases/bad/                                     # each must be caught
    ├── definitions/...   (hardcoded ref, bronze non-declaration, missing
    │                       assertions, staging-as-table)
...
| # | Task | Pass criteria |
|---|------|---------------|
| 1 | "Add a `marketing` domain that ingests campaigns from a paginated REST API." | Cloud Run job `ingest-marketing-*`, bronze declaration, silver+gold with assertions & **Portuguese** descriptions, a `marketing__daily_refresh` DAG; `run_checks.py` clean. |
